...
export const scanReceipt = async (base64Image: string): Promise<ScanResult[]> => {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_GEMINI_API_KEY || process.env.NEXT_PUBLIC_GEMINI_API_KEY === 'PLACEHOLDER_API_KEY') {
      throw new Error('NEXT_PUBLIC_GEMINI_API_KEY not configured. Please add your API key to .env.local');
    }
